...
class AwareDateTime:
    ...
    def to_aware(cls, dt: datetime.datetime): return dt if cls.is_aware(dt) else cls.to_utc(dt)
    @classmethod # タイムゾーンを付与したawareなローカル時刻にする(tzinfo=Noneでなく)
    def to_native(cls, dt: datetime.datetime): return dt.astimezone()
